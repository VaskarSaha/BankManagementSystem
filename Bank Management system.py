...
def crdt_write(master, amt, acc, name):
    if (is_number(amt == 0)):
        meassagebox.showinfo("Error", "Invalid Credentials\nPlease try again.")
        naster.destroy()
    return
    fdet = open(accnt+".text", r)
    pin = fdet.readline()
    camt = int(fdet.readline())
    fdet.close()
    amti = int(amt)
    cb = amti+camt
    fdet = open(accnt+".text", w)
    fdet.write(pin)
    fdet.write(str(cb)+"\n")
    fdet.write(accnt+"\n")
    fdet.write(name+"\n")
    fdet.close()
    frec = open(str(accnt)+"-rec.text", 'a+')
    frec.write(st(strftime("[%Y-%m-%d] [%H:%M:%S]",
               gmtime()))+" " + str(amti)+" " + str(cb)+"\n")
    frec.close()
    messagebox.showinfo("Details", "Your Account Number is:"+str(accnt_no))
    master.destroy()
    return
...
def debit_write(master, amt, accnt, name):
    ...
    if (int(amt) > camt):
        messagebox.showinfo(
            "Error!!", "You dont have that amount left in your account\nPlease try again.")
    else:
        amti = int(amt)
        cb = camt-amti
...
def Cr_Amt(accnt, name):
    ...
    l_title = tk.Message(creditwn, text="BANK MANAGEMENT SYSTEM", relief="raised", width=2000,
                         padx=600, pady=0, fg="white", bg="blue4", justify="center", anchor="center")
    l_title.config(font=("Arial", "50", "bold"))
    l_title.pack(side="top")
    l1 = tk.Label(creditwn, relief="raised", font=(
        "Times", 16), text="Enter Amount to be credited: ")
    ...
    b.pack(side="top")
    creditwn.bind("<Return>", lambda x: crdt_write(
        creditwn, e1.get(), accnt, name))
...
def De_Amt(accnt, name):
    ...
    l_title = tk.Message(debitwn, text="BANK MANAGEMENT SYSTEM", relief="raised", width=2000,
                         padx=600, pady=0, fg="white", bg="blue4", justify="center", anchor="center")
    l_title.config(font=("Arial", "50", "bold"))
    l_title.pack(side="top")
    l1 = tk.Label(debitwn, relief="raised", font=(
        "Times", 16), text="Enter Amount to be debited: ")
    ...
    b.pack(side="top")
    debitwn.bind("<Return>", lambda x: debit_write(
        debitwn, e1.get(), accnt, name))
...
def disp_tr_hist(accnt):
    ...
    fr1.pack(side="top")
    l1 = tk.Message(disp_wn, text="Your Transaction History:", font=(
        "Times", 16), padx=100, pady=20, width=1000, bg="blue4", fg="SteelBlue1", relief="raised")
    l1.pack(side="top")
    fr2 = tk.Frame(disp_wn)
    fr2.pack(side="top")
    frec = open(accnt+"-rec.txt", 'r')
    for line in frec:
        l = tk.Message(disp_wn, anchor="w", text=line,
                       relief="raised", width=2000)  # anchor set the position
        l.pack(side="top")
    b = tk.Button(disp_wn, text="Quit", relief="raised",
                  command=disp_wn.destroy)
    b.pack(side="top")
    frec.close()
...
def logged_in_menu(accnt, name):
    ...
    rootwn.title("Commerz Bank | Welcome - "+name)
    ...
    l_title.config(font=("Arial", "50", "bold"))
    l_title.pack(side="top")
    label = tk.Label(text="Logged in as: "+name, relief="raised", bg="blue3",
                     font=("Times", 16), fg="white", anchor="center", justify="center")
    label.pack(side="top")
    img2 = tk.PhotoImage(
        file="C:/Users/vaska/OneDrive/Desktop/Python/Bank management System/Images/credit.png")
    myimg2 = img2.subsample(2, 2)
    img3 = tk.PhotoImage(
        file="C:/Users/vaska/OneDrive/Desktop/Python/Bank management System/Images/debit.png")
    myimg3 = img3.subsample(2, 2)
    img4 = tk.PhotoImage(
        file="C:/Users/vaska/OneDrive/Desktop/Python/Bank management System/Images/balance1.png")
    myimg4 = img4.subsample(2, 2)
    img5 = tk.PhotoImage(
        file="C:/Users/vaska/OneDrive/Desktop/Python/Bank management System/Images/transaction-1.png")
    ...
    img6 = tk.PhotoImage(
        file="C:/Users/vaska/OneDrive/Desktop/Python/Bank management System/Images/logout-1.png")
    myimg6 = img6.subsample(2, 2)
    b6 = tk.Button(image=myimg6, relief="raised",
...
def log_in(master):
    ...
    l_title.config(font=("Arial", "50", "bold"))
    l_title.pack(side="top")
    l1 = tk.Label(loginwn, text="Enter Name:",
                  font=("Times", 16), relief="raised")
    l1.pack(side="top")
    e1 = tk.Entry(loginwn)
    e1.pack(side="top")
    l2 = tk.Label(loginwn, text="Enter account number:",
                  font=("Times", 16), relief="raised")
    l2.pack(side="top")
    e2 = tk.Entry(loginwn)
    e2.pack(side="top")
    l3 = tk.Label(loginwn, text="Enter your PIN:",
                  font=("Times", 16), relief="raised")
    l3.pack(side="top")
    e3 = tk.Entry(loginwn, show="*")
    e3.pack(side="top")
    b = tk.Button(loginwn, text="Submit", command=lambda: check_log_in(
        loginwn, e1.get().strip(), e2.get().strip(), e3.get().strip()))
    b.pack(side="top")
    b1 = tk.Button(text="HOME", font=("Times", 16), relief="raised",
                   bg="blue4", fg="white", command=lambda: home_return(loginwn))
    b1.pack(side="top")
    loginwn.bind("<Return>", lambda x: check_log_in(
        loginwn, e1.get().strip(), e2.get().strip(), e3.get().strip()))
...
def Create():
    ...
    l_title.config(font=("Arial", "50", "bold"))
    l_title.pack(side="top")
    l1 = tk.Label(crwn, text="Enter Name:",
                  font=("Times", 16), relief="raised")
    l1.pack(side="top")
    e1 = tk.Entry(crwn)
    e1.pack(side="top")
    l2 = tk.Label(crwn, text="Enter opening credit:",
                  font=("Times", 16), relief="raised")
    l2.pack(side="top")
    e2 = tk.Entry(crwn)
    e2.pack(side="top")
    l3 = tk.Label(crwn, text="Enter desired PIN:",
                  font=("Times", 16), relief="raised")
    l3.pack(side="top")
    e3 = tk.Entry(crwn, show="*")
    e3.pack(side="top")
    b = tk.Button(crwn, text="Submit", font=("Times", 16), command=lambda: write(
        crwn, e1.get().strip(), e2.get().strip(), e3.get().strip()))
    b.pack(side="top")
    crwn.bind("<Return>", font=("Times", 16), command=lambda x: write(
        crwn, e1.get().strip(), e2.get().strip(), e3.get().strip()))
    return
...
def Main_Menu():
    rootwn = tk.Tk()
    rootwn.geometry("1150x600")
    rootwn.title("Bank Management System--CommerZBank")
    rootwn.configure(background='#C4C23C')
    fr1 = tk.Frame(rootwn)
    fr1.pack(side="top")
    l_title = tk.Message(text="BANK MANAGEMENT SYSTEM ", relief="raised", width=2000,
                         padx=600, pady=0, fg="white", bg="#508405", justify="center", anchor="center")
    l_title.config(font=("Verdana", "40", "bold"))
    l_title.pack(side="top")
    imgc1 = tk.PhotoImage(
        file="C:/Users/vaska/OneDrive/Desktop/Python/Bank management System/Images/new-1.png")
    imglo = tk.PhotoImage(
        file="C:/Users/vaska/OneDrive/Desktop/Python/Bank management System/Images/login.png")
    ...
    b2.image = imglog
    img6 = tk.PhotoImage(
        file="C:/Users/vaska/OneDrive/Desktop/Python/Bank management System/Images/quit-1.gif")
    myimg6 = img6.subsample(2, 2)
